...
var cards = (function() {
    ...
    var zIndexCounter = 1;
    var all = []; //All the cards created.410
    var start = 1;
    var end = start + 14;
    ...
    Card.prototype = {
        init: function(suit, rank, table) {
            ...
            this.el = $('<div/>').css({
                ...
                position: 'absolute',
                cursor: 'pointer',
            }).addClass('card').data('card', this).click(mouseEvent).appendTo($(table));
            this.showCard();
            this.moveToFront();
            //$('.card').click(mouseEvent);
        },
        ...
        showCard: function() {
            ...
            xpos = (-rank + 1 - hOffset) * opt.cardSize.width;
            ...
            if (rank === 15) {
                ...
                xpos = -opt.cardSize.width * (5 + wcOffset);
            } else if (rank === 14) {
                ypos = 0;
                let wcOffset;
                switch(this.suit) {
                ...
                xpos = -opt.cardSize.width * wcOffset;

            } else if (rank >= (13 - hOffset)) {
                ypos -= opt.cardSize.height;
                xpos -= 18;
            }
            this.rotate(0);
            $(this.el).css('background-position', xpos + 'px ' + ypos + 'px').css('border-radius', '8px');
        },
    ...
    Container.prototype.extend({
        //ADDED ADDITIONAL PROPERTY FOR INDIVIDUAL HAND PADDING
        //iPadding: opt.cardSize.padding,
        ...
        init: function(options) {
            ...
            this.iPadding = options.iPadding || opt.cardSize.padding;
    ...
    Hand.prototype.extend({
        calcPosition: function(options) {
            options = options || {};
            var width = opt.cardSize.width + (this.length - 1) * this.iPadding;
            var left = Math.round(this.x - width) + ((this.length * this.iPadding) / 2);
            var top = Math.round(this.y - opt.cardSize.height / 2);
            for (var i = 0; i < this.length; i++) {
                this[i].targetTop = top;
                this[i].targetLeft = left + i * this.iPadding;
            }
        },
